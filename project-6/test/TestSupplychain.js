...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async() => {
        ...
        // Declare and Initialize a variable for event
        let eventEmitted = false
        const balanceFarmerBefore = await getBalance(originFarmerID)
        const balanceDistributorBefore = await getBalance(distributorID)
        const gasPrice = toBN(await getGasPrice())
        ...
        assert.equal(
            toBN(balanceFarmerAfter)
                .sub(toBN(balanceFarmerBefore))
                .eq(toBN(productPrice)),
            true,
            "Invalid farmer balance before and after"
        )
        assert.equal(
            toBN(balanceDistributorBefore)
                .sub(toBN(productPrice))
                .sub(txFee)
                .eq(toBN(balanceDistributorAfter)),
            true,
            "Invalid distributor balance before and after"
        )
    })
